...
export class FirmwareValidator {
  ...
  private static validateManifestEntry(entry: unknown, index: number, errors: ValidationError[], warnings: ValidationError[]): void {
    if (!entry || typeof entry !== 'object') {
      errors.push({ field: `files[${index}]`, message: `File entry at index ${index} is not an object.`, severity: 'error' });
      return;
    }
    const f = entry as Partial<ManifestFileEntry>;
    if (!f.path || typeof f.path !== 'string') {
      errors.push({ field: `files[${index}].path`, message: 'File entry is missing a valid path string.', severity: 'error' });
    } else if (f.path.includes('..') || f.path.startsWith('/') || f.path.includes('://')) {
      errors.push({ field: `files[${index}].path`, message: `Unsafe path detected in manifest: "${f.path}".`, severity: 'error' });
    }
  ...
  public static validatePackage(pkg: FirmwarePackage, detectedCapacityBytes?: number | null, expectedChip = 'ESP32-S3'): ValidationResult {
    ...
    const warnings: ValidationError[] = [];
    if (!pkg.files?.length) {
      errors.push({ field: 'files', message: 'Firmware package contains no binary files.', severity: 'error' });
      return { isValid: false, errors, warnings, flashCapacityStatus: 'unknown' };
    }
  ...
  public static parseFlashCapacityBytes(flashSizeStr?: string): number | null {
    ...
    const clean = flashSizeStr.toUpperCase().replace(/\s/g, '');
