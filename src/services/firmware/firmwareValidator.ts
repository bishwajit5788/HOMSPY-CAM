...
export class FirmwareValidator {
  public static validateManifestSchema(manifest: unknown): ValidationResult {
    ...
    if (!manifest || typeof manifest !== 'object') {
      errors.push({ field: 'manifest', message: 'Manifest must be a valid JSON object.', severity: 'error' });
    ...
    if (!m.files || !Array.isArray(m.files) || m.files.length === 0) {
      errors.push({ field: 'files', message: 'Manifest must declare a non-empty "files" array.', severity: 'error' });
      return { isValid: false, errors, warnings, flashCapacityStatus: 'unknown' };
    }
  ...
  private static validateManifestEntry(entry: unknown, index: number, errors: ValidationError[], warnings: ValidationError[]): void {
    ...
    if (!f.path || typeof f.path !== 'string') errors.push({ field: `files[${index}].path`, message: 'File entry is missing a valid path string.', severity: 'error' });
    else if (f.path.includes('..') || f.path.startsWith('/') || f.path.includes('://')) errors.push({ field: `files[${index}].path`, message: `Unsafe path detected in manifest: "${f.path}".`, severity: 'error' });
  ...
  public static validatePackage(pkg: FirmwarePackage, detectedCapacityBytes?: number | null, expectedChip = 'ESP32-S3'): ValidationResult {
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      ...
      if (!Number.isSafeInteger(end) || end < sorted[i].offsetNum) errors.push({ field: 'address_overflow', message: `Address range overflow detected for "${sorted[i].fileName}".`, severity: 'error' });
      else if (end > sorted[i + 1].offsetNum) errors.push({ field: 'address_overlap', message: `Memory overlap detected between "${sorted[i].fileName}" and "${sorted[i + 1].fileName}".`, severity: 'error' });
    }
    ...
    if (!detectedCapacityBytes || detectedCapacityBytes <= 0) {
      ...
      for (const bin of pkg.files) {
        ...
        if (!Number.isSafeInteger(end) || end < bin.offsetNum) { flashCapacityStatus = 'exceeded'; errors.push({ field: 'flash_capacity', message: `Address overflow detected for "${bin.fileName}".`, severity: 'error' }); }
        else if (end > detectedCapacityBytes) { flashCapacityStatus = 'exceeded'; errors.push({ field: 'flash_capacity', message: `File "${bin.fileName}" exceeds detected flash capacity.`, severity: 'error' }); }
      }
    }
