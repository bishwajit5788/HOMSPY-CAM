...
export class FlashService {
  ...
  private setState(newState: DeviceState, message: string, error?: string, troubleshooting: string[] = []): void {
    if (!isValidTransition(this.currentState, newState)) {
      ...
      this.stateMessage = `Illegal state transition attempted from ${from} to ${newState}.`;
      ...
      logService.addLog(`State machine guarded: ${from} -> ${newState} is not permitted.`, 'error');
    } else {
  ...
  public async eraseFlash(): Promise<void> {
    ...
    const opId = this.nextOperationId();
    this.isOperationActive = true;
    this.setState('ERASING', 'Transmitting full chip erase command...');
    this.updateProgress({ stage: 'erasing', stageText: 'Erasing target flash memory...', percentage: 20 });
    try {
      await esp32Service.eraseChip();
      if (!this.isCurrentOperation(opId)) return;
      this.updateProgress({ stage: 'complete', stageText: 'Chip erase complete! Ready for firmware flashing.', percentage: 100 });
      this.setState('BOOTLOADER_READY', 'Full flash memory successfully erased. Bootloader ready.');
    } catch (err: unknown) {
      if (this.isCurrentOperation(opId)) this.setState('ERROR', 'Failed to erase flash memory.', err instanceof Error ? err.message : String(err));
      throw err;
    } finally {
      if (this.isCurrentOperation(opId)) this.isOperationActive = false;
    }
  ...
  public async startFlashing(pkg: FirmwarePackage, config: FlashConfig): Promise<void> {
    ...
    if (detectedCapacityBytes === null) {
      const message = 'Physical flash capacity is UNKNOWN. Flashing is blocked because memory boundary safety cannot be verified.';
      this.setState('ERROR', 'Flashing blocked: flash capacity unknown.', message, ['Reconnect the board and ensure SPI flash ID detection succeeds.', 'Do not substitute the UI flash-size setting for detected hardware capacity.']);
      this.isOperationActive = false;
      throw new Error(message);
    }

    const validationResult = FirmwareValidator.validatePackage(pkg, detectedCapacityBytes, this.detectedChip.chipName || 'ESP32-S3');
    pkg.flashCapacityStatus = validationResult.flashCapacityStatus;
    if (!validationResult.isValid || validationResult.flashCapacityStatus !== 'verified') {
      const errorSummary = validationResult.errors.map((e) => e.message).join('\n') || 'Flash capacity validation did not reach a verified state.';
      this.setState('ERROR', 'Firmware package validation failed before flashing.', errorSummary, ['Verify the package checksums and executable headers.', 'Ensure all segments fit inside the detected physical flash capacity.']);
      this.isOperationActive = false;
    ...
    try {
      ...
      await esp32Service.flashImages(
        pkg.files,
        { ...config, flashSize: (detectedFlashSize || config.flashSize) as FlashConfig['flashSize'] },
        (fileIdx, written, _total, fileName) => {
          if (!this.isCurrentOperation(opId)) return;
  ...
  public async disconnect(): Promise<void> {
    this.invalidateCurrentOperation();
    this.isOperationActive = false;
    try { await esp32Service.disconnect(); } finally {
      await portCoordinator.releaseLease('flasher');
      this.activePort = null;
      this.detectedChip = null;
      if (this.currentState !== 'DISCONNECTED') this.setState('DISCONNECTING', 'Disconnecting device...');
      if (this.currentState !== 'DISCONNECTED') this.setState('DISCONNECTED', 'Ready to connect.');
    }
  }
